...
import { collectBrightDataRetailers } from './brightdata.js';
...
if(command==='collect:brightdata'){
  const result=await collectBrightDataRetailers();recordCollection(db,result);
  console.log(JSON.stringify({source:result.source,status:result.status,observations:result.observations.length,matched_products:result.matched_products,errors:result.errors},null,2));
