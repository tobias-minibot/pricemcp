...
export function buildApp(db=openDatabase(),options:{readOnly?:boolean}={}){
  ...
  app.get('/cheap-apples',(_req,reply)=>{
    ...
    const products=activeProducts.map(product=>{const price=prices.get(product.id);return{product_id:product.id,name:product.name,category:product.category,family:product.family,attributes:product.attributes,offer:price.best_trusted_offer,savings_minor:price.savings_vs_official_minor,official_total_minor:price.official_price?.total_minor}}).sort((a,b)=>Number(Boolean(b.offer))-Number(Boolean(a.offer))||a.category.localeCompare(b.category)||a.name.localeCompare(b.name));
