...
const amountMinor=(value:unknown):number|null=>{if(value===null||value===undefined||(typeof value==='string'&&!value.trim()))return null;const parsed=Number(value);return Number.isFinite(parsed)?Math.round(parsed*100):null};
...
async function duffelFlightOffers(subject:FlightPriceSubject):Promise<FlightProviderResult>{
  ...
  const offers=items.map((item:any,index:number):UniversalOffer=>{
    ...
    const itineraryKey=segments.map((segment:any)=>[
    ...
    const expiresAt=typeof item.expires_at==='string'?item.expires_at:null,expired=expiresAt!==null&&!Number.isNaN(Date.parse(expiresAt))&&Date.parse(expiresAt)<=Date.now();
    const flight:FlightOfferDetails={comparison_key:comparisonKey,cabin:String(subject.cabin||'economy'),fare_brands:fareBrands,baggage:bags,base_minor:amountMinor(item.base_amount),base_currency:typeof item.base_currency==='string'?item.base_currency:null,tax_minor:amountMinor(item.tax_amount),tax_currency:typeof item.tax_currency==='string'?item.tax_currency:null,emissions_kg:emissions!==null&&Number.isFinite(emissions)?emissions:null,total_duration_minutes:totalDuration,change_before_departure:changeRule,refund_before_departure:refundRule,slices:normalizedSlices};
    return {offer_id:`duffel:${item.id||index}`,dataset:item.live_mode===true?'duffel-live':'duffel-test',synthetic:item.live_mode!==true,provider:{provider_id:String(owner.id||'duffel'),name:String(owner.name||'Duffel airline offer'),trusted:true,trust_score:null},quote:{amount_minor:amount,currency:String(item.total_currency||'USD'),shipping_minor:0,total_minor:amount,basis:'Duffel offer total including provider-reported base fare and taxes; optional services may cost extra'},availability:expired?'unavailable':'available',conditions,match:{canonical:true,confidence:1},source:{method:item.live_mode===true?'duffel_flights_live':'duffel_flights_test',source_product_id:String(item.id||index),url:safeSourceUrl('https://duffel.com/docs/api/v2/offer-requests')},observed_at:new Date().toISOString(),freshness:{age_seconds:0,status:'fresh'},expires_at:expiresAt,flight};
  }).filter((offer:UniversalOffer)=>offer.quote.total_minor>0&&offer.availability==='available').sort((a:UniversalOffer,b:UniversalOffer)=>a.quote.total_minor-b.quote.total_minor).slice(0,50);
